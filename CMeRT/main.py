        ...
        for phase in cfg.SOLVER.PHASES:
            ...
            with torch.set_grad_enabled(training):
                ...
                for batch_idx, data in enumerate(pbar, start=1):
                    ...
                    if cfg.LOSS.NONUNIFORM:
                        det_target = det_target[:, -1:, :].contiguous()
                        if cfg.LOSS.DET_WEIGHT:
                            det_target = F.pad(det_target, (0, 0, cfg.MODEL.LSTR.WORK_MEMORY_NUM_SAMPLES-1, 0), 'replicate')
                    ...
                    for i, detant_score in enumerate(scores):
                        if cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES > 0:
                            ...
                            if cfg.LOSS.NONUNIFORM:
                                det_score = det_score[:, -1:, :].contiguous()
                                if cfg.LOSS.DET_WEIGHT:
                                    det_score = F.pad(det_score, (0, 0, cfg.MODEL.LSTR.WORK_MEMORY_NUM_SAMPLES-1, 0), 'replicate')    
                            det_score = det_score.reshape(-1, cfg.DATA.NUM_CLASSES)
                            ant_score = ant_score.reshape(-1, cfg.DATA.NUM_CLASSES)

                        if cfg.LOSS.NONUNIFORM:
                            detant_score = detant_score[:, (cfg.MODEL.LSTR.WORK_MEMORY_NUM_SAMPLES-1):, :].contiguous()
                            if cfg.LOSS.DET_WEIGHT:
                                detant_score = F.pad(detant_score, (0, 0, cfg.MODEL.LSTR.WORK_MEMORY_NUM_SAMPLES-1, 0), 'replicate')    
